...
func retryPolicyConfigFields() []*service.ConfigField {
	return []*service.ConfigField{
		...
		service.NewStringListField("non_retryable_error_types").
			Description("Non-Retriable errors. This is optional. Temporal server will stop retry if error type matches this list. Note that cancellation is not a failure, so it won't be retried and only StartToClose or Heartbeat timeouts are retryable.").
			Optional(),
	}
}
